...
interface CommunityPostRespons {
  ok: boolean;
  post: PostWithUser;
  isWondering: boolean;
}
...
const CommunityPostDetail: NextPage = () => {
  ...
  const {data, mutate} = useSWR<CommunityPostRespons>(router.query.id ? `/api/posts/${router.query.id}` : null);
  const { register, handleSubmit, reset } = useForm<AnswerForm>();
  const [wonder, { loading }] = useMutation(`/api/posts/${router.query.id}/wonder`);
  const [sendAnswers, { data:answerData, loading:answerLoading }] = useMutation(`/api/posts/${router.query.id}/answers`);
  const onWonderClick = () => {
    if(!data) return;
    mutate(
      {
